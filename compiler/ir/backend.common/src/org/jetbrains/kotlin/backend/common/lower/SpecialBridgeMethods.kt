...
class SpecialBridgeMethods(val irBuiltIns: IrBuiltIns) {
    ...
    fun getSpecialMethodInfo(irFunction: IrSimpleFunction): SpecialMethodWithDefaultInfo? {
        // native uses bridges machinery with top-level function as target to create a wrapper for a native function pointer.
        // They are not special, let's completely ignore them.
        if (irFunction.dispatchReceiverParameter == null) return null
